...
import com.example.mesurmentconverter.R;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        search.setOnClickListener(clik ->
        {
            try {
                ...
                EditText searchteam = (EditText) findViewById(R.id.input_team);
                resp = getRequest.execute(myUrl + searchteam.getText().toString().toUpperCase()).get();
                for (Iterator<String> iter = resp.keys(); iter.hasNext(); ) {
                    String key = iter.next();
                    soccerList.add(new SoccerObject(key, resp.getString(key), searchteam.getText().toString().toUpperCase()));
                }
